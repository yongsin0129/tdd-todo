...
describe('Express App Initialization', () => {
    ...
    describe('Middleware Configuration', () => {
        ...
        it('should have CORS enabled', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).options('/health').set('Origin', 'http://localhost:3000');
            // Assert
            expect(response.headers['access-control-allow-origin']).toBeDefined();
        });
        it('should set proper CORS headers on requests', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).get('/health').set('Origin', 'http://localhost:3000');
            // Assert
    ...
    describe('Error Handling', () => {
        ...
        it('should return 404 for POST to unknown routes', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).post('/unknown-route');
            // Assert
            expect(response.status).toBe(404);
        });
        it('should return JSON error response for 404', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).get('/non-existent');
            // Assert
            expect(response.headers['content-type']).toMatch(/application\/json/);
        ...
        it('should include requested path in 404 error message', async () => {
            ...
            // Act
            const response = await (0, supertest_1.default)(app).get(testPath);
            // Assert
    ...
    describe('HTTP Methods Support', () => {
        ...
        it('should handle POST requests', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).post('/unknown');
            // Assert
            expect(response.status).toBe(404); // Not found, but POST method is handled
        });
        it('should handle PUT requests', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).put('/unknown');
            // Assert
            expect(response.status).toBe(404); // Not found, but PUT method is handled
        });
        it('should handle DELETE requests', async () => {
            // Arrange & Act
            const response = await (0, supertest_1.default)(app).delete('/unknown');
            // Assert
...
describe('Server Lifecycle', () => {
    ...
    describe('Server Shutdown', () => {
        ...
        it('should allow multiple close calls without errors', async () => {
            ...
            const secondClose = new Promise((resolve) => {
